import { Observable } from 'rxjs/Observable';
import { GSPMetadataRTService } from '../metadata.rtservice';
export declare class MetadataRTService {
    private gspMetadataService;
    constructor(gspMetadataService: GSPMetadataRTService);
    metadatas: any[];
    addMetadataRT(metadata: any): void;
    clearMetadataRT(): void;
    GetMetadataRTList(typeList: string): Observable<any>;
    GetMetadataRTListForHelp(typeList: string): Observable<any>;
    getMetadataListByBizTypeId(bizTypeId: string, metadataTypes: string): Observable<any>;
    getMetadataListByBizTypeIdForHelp(boList: string, bizTypeId: string, metadataTypes: string): Observable<any>;
    GetMetadataRT(metadataID: string): Observable<any>;
}
