...
declare class AttachmentService {
     ...
     * 构造函数
     */
    constructor(viewModelContext: ViewModelContext, entityService: EntityService, removeDataService: RemoveDataService, attachDataService: AttachmentDataService);
    /**
     * 上传单个文件
     ...
     * 上传多个文件
     */
    uploadAndBatchAddRows(attachmentInfoFieldPath: string, rootDirId?: string, parentDirName?: string): Observable<any>;
    /**
     * 批量删除附件所在的行
     */
    removeAttachmentRows(attachmentInfoFieldPath: string): Observable<any>;
    /**
     * 从上下文中中获取控件传递的附件信息
     */
     ...
     * 获取要删除的附件对应的数据id数组
     */
    getDataIdsToRemove(attachmentInfoFieldPath: string): string[];
    /**
     * 从命令上下文中获取要删除附件ids
     */
    getAttachmentIdsToRemoveFromContext(): string[];
    /**
     * 根据路径获取附件字段值数组
