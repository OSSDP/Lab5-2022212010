...
exports.EditorUi = void 0;
var tslib_1 = require("tslib");
var Actions_1 = require("../action/Actions");
var mxgraph_ts_1 = require("@edp-pmf/mxgraph-ts");
var DefaultToolbarItems_1 = require("../toolbar/DefaultToolbarItems");
var DefaultToolBox_1 = require("../toolbox/DefaultToolBox");
...
var MxGraphInnerClass_1 = require("../util/MxGraphInnerClass");
/**
 * 设计器统一入口，持有各部分组件实例，可触发以及响应事件
 */
var EditorUi = /** @class */ (function (_super) {
    tslib_1.__extends(EditorUi, _super);
    /**
     * 构造方法
    ...
    function EditorUi(container, eventSource, importLinks) {
         ...
         * 分割条宽度
         */
        _this.sidebarSplitSize = (mxgraph_ts_1.mxClient.IS_TOUCH || mxgraph_ts_1.mxClient.IS_POINTER) ? 12 : 8;
        /**
         * 画布最小宽度
         */
         ...
         * 0偏移量
         */
        _this.zeroOffset = new mxgraph_ts_1.mxPoint(0, 0);
        /**
         * 是否显示菜单栏
         */
    ...
    EditorUi.prototype.show = function () {
        ...
        this.resetToolbox();
        // 禁用右键快捷菜单
        mxgraph_ts_1.mxEvent.disableContextMenu(this.container);
        // 点击非当前菜单位置时关闭菜单
        this.gestureHandler = function (evt) {
            if (_this.currentMenu != null && mxgraph_ts_1.mxEvent.getSource(evt) != _this.currentMenu.div) {
                _this.hideCurrentMenu();
            }
        };
        mxgraph_ts_1.mxEvent.addGestureListeners(document, this.gestureHandler);
        ...
        mxgraph_ts_1.mxEvent.addListener(window, 'resize', this.resizeHandler);
        this.addListener(PmfEvents_1.PmfEvents.ResetGraphView, function () {
            _this.resetScrollbars();
        });
        this.fireEvent(new mxgraph_ts_1.mxEventObject(PmfEvents_1.PmfEvents.ResetGraphView));
    ...
    EditorUi.prototype.refresh = function (sizeDidChange) {
        ...
        var quirks = mxgraph_ts_1.mxClient.IS_QUIRKS;
        ...
        var off = 0;
        if (mxgraph_ts_1.mxClient.IS_IOS && !window.navigator.standalone) {
        ...
        if (tmp > 0 && !mxgraph_ts_1.mxClient.IS_QUIRKS) {
            tmp += 1;
        }
        ...
        if (sizeDidChange && this.graph != null) {
            ...
            this.fireEvent(new mxgraph_ts_1.mxEventObject(PmfEvents_1.PmfEvents.ResizeGraphView));
        }
    };
    /**
     * 创建各容器div
    ...
    EditorUi.prototype.createDivs = function () {
        ...
        this.sidebarSplit.setAttribute('title', mxgraph_ts_1.mxResources.get('collapseExpand'));
        ...
        this.footerContainer.style.zIndex = (mxgraph_ts_1.mxPopupMenu.prototype.zIndex - 2).toString();
    ...
    EditorUi.prototype.addSplitHandler = function (elt, horizontal, dx, onChange) {
        var _this = this;
        onChange = mxgraph_ts_1.mxUtils.bind(this, onChange);
        var start;
        var initial;
        var ignoreClick = true;
        var last;
        // Disables built-in pan and zoom in IE10 and later
        if (mxgraph_ts_1.mxClient.IS_POINTER) {
        ...
        var clickHandler = function (evt) {
            if (!ignoreClick) {
                ...
                last = getValue();
                onChange(next);
                mxgraph_ts_1.mxEvent.consume(evt);
            }
        };
        var startHandler = function (evt) {
            start = new mxgraph_ts_1.mxPoint(mxgraph_ts_1.mxEvent.getClientX(evt), mxgraph_ts_1.mxEvent.getClientY(evt));
            initial = getValue();
            ignoreClick = false;
            mxgraph_ts_1.mxEvent.consume(evt);
        };
        var moveHandler = function (evt) {
            if (start != null) {
                var pt = new mxgraph_ts_1.mxPoint(mxgraph_ts_1.mxEvent.getClientX(evt), mxgraph_ts_1.mxEvent.getClientY(evt));
        ...
        mxgraph_ts_1.mxEvent.addListener(elt, 'click', clickHandler);
        mxgraph_ts_1.mxEvent.addGestureListeners(elt, startHandler, null, null);
        mxgraph_ts_1.mxEvent.addGestureListeners(document, null, moveHandler, dropHandler);
    };
    /**
     * 分割条移动后刷新界面
     * @param value
    ...
    EditorUi.prototype.beforeCreateGraph = function () {
        mxgraph_ts_1.mxConstants.DEFAULT_VALID_COLOR = '#FFFFFF';
        mxgraph_ts_1.mxConstants.VALID_COLOR = '#00CD66';
        // 引导线颜色
        mxgraph_ts_1.mxConstants.GUIDE_COLOR = '#0088cf';
        // 选中高亮框宽度
        mxgraph_ts_1.mxConstants.HIGHLIGHT_STROKEWIDTH = 1;
        //设置节点选中线框宽度
        mxgraph_ts_1.mxConstants.VERTEX_SELECTION_STROKEWIDTH = 1;
        mxgraph_ts_1.mxConstants.VERTEX_SELECTION_COLOR = '#0000CD';
        mxgraph_ts_1.mxConstants.VERTEX_SELECTION_DASHED = true;
        mxgraph_ts_1.mxGuide.prototype.createGuideShape = function (horizontal) {
            var guide = new mxgraph_ts_1.mxPolyline([], mxgraph_ts_1.mxConstants.GUIDE_COLOR, mxgraph_ts_1.mxConstants.GUIDE_STROKEWIDTH);
            return guide;
        };
    };
    /**
    ...
    EditorUi.prototype.createGraphInstance = function () {
        var graph = new Graph_1.Graph(this, this.diagramContainer);
        return graph;
    };
    /**
     * 创建画布后
    ...
    EditorUi.prototype.afterCreateGraph = function (graph) {
        var _this = this;
        new mxgraph_ts_1.mxRubberband(graph);
        graph.setDropEnabled(true);
        graph.setConnectable(true);
        // 设置画布容器dom可以获得焦点
        graph.container.setAttribute('tabindex', '0');
        //选中元素变更后出发EditorUI选中模型变更事件，修改action状态
        graph.getSelectionModel().addListener(mxgraph_ts_1.mxEvent.CHANGE, function (sender, evt) {
            _this.fireEvent(new mxgraph_ts_1.mxEventObject(PmfEvents_1.PmfEvents.SelectionModelChanged));
        });
        // 保证鼠标按下时焦点一直是画布容器
        var graphFireMouseEvent = graph.fireMouseEvent;
        graph.fireMouseEvent = function (evtName, me, sender) {
            if (evtName == mxgraph_ts_1.mxEvent.MOUSE_DOWN) {
        ...
        graph.sizeDidChange();
        // 重写复制方法
        mxgraph_ts_1.mxClipboard.copy = function (graph, cells) {
            cells = cells || graph.getSelectionCells();
            // 仅复制节点
            cells = cells.filter(function (cell) { return cell.vertex; });
            var result = graph.getExportableCells(graph.model.getTopmostCells(cells));
            mxgraph_ts_1.mxClipboard.insertCount = 1;
            mxgraph_ts_1.mxClipboard.setCells(graph.cloneCells(result));
            return result;
        };
        // 重写粘贴方法
        mxgraph_ts_1.mxClipboard.paste = function (graph) {
            var cells = null;
            if (!mxgraph_ts_1.mxClipboard.isEmpty()) {
                cells = graph.getImportableCells(mxgraph_ts_1.mxClipboard.getCells());
                // 仅粘贴节点
                cells = cells.filter(function (cell) { return cell.vertex; });
                var delta_1 = mxgraph_ts_1.mxClipboard.insertCount * mxgraph_ts_1.mxClipboard.STEPSIZE;
                ...
                try {
                    ...
                    mxgraph_ts_1.mxClipboard.insertCount++;
                }
                finally {
    ...
    EditorUi.prototype.createActions = function () {
        var actions = new Actions_1.Actions(this);
        return actions;
    };
    /**
     * 创建工具栏项列表
    ...
    EditorUi.prototype.createToolbarItems = function () {
        var toolbarItems = new DefaultToolbarItems_1.DefaultToolbarItems();
        return toolbarItems;
    };
    /**
     * 创建键盘事件处理器
    ...
    EditorUi.prototype.createKeyHandler = function (graph) {
        var keyHandler = new KeyHandler_1.KeyHandler(graph);
        return keyHandler;
    };
    /**
     * 创建菜单栏
    ...
    EditorUi.prototype.createMenubar = function () {
        var menubar = new Menubar_1.Menubar(this, this.menubarContainer);
        return menubar;
    };
    /**
     * 创建工具栏
    ...
    EditorUi.prototype.createToolbar = function () {
        var toolbar = new Toolbar_1.Toolbar(this, this.toolbarContainer);
        return toolbar;
    };
    /**
     * 创建侧边栏
    ...
    EditorUi.prototype.createSidebar = function () {
        var sidebar = new Sidebar_1.Sidebar(this, this.sidebarContainer);
        return sidebar;
    };
    /**
     * 创建内置属性框
    ...
    EditorUi.prototype.createFormat = function () {
        var format = new Format_1.Format(this, this.formatContainer);
        return format;
    };
    /**
     * 创建底部栏
    ...
    EditorUi.prototype.createFooter = function () {
        var footer = new Footer_1.Footer(this, this.footerContainer);
        return footer;
    };
    /**
     * 重置工具条
    ...
    EditorUi.prototype.createToolbox = function () {
        var toolbox = new DefaultToolBox_1.DefaultToolbox();
        return toolbox;
    };
    /**
     * 加载BpmnModel
    ...
    EditorUi.prototype.addBpmnListeners = function () {
        var _this = this;
        // 元素修改事件处理
        this.graph.getModel().addListener(mxgraph_ts_1.mxEvent.CHANGE, function (sender, evt) {
            ...
            changes && changes.forEach(function (chg) {
                ...
                if (chg.constructor.name == MxGraphInnerClass_1.MxGraphInnerClass.mxTerminalChange) {
                    var change = chg;
                    ...
                    if (flag && edge != null) {
                        _this.fireEvent(new mxgraph_ts_1.mxEventObject(PmfEvents_1.PmfEvents.ShowProperty, "cells", [edge]));
                        flag = false;
                    }
                    _this.graph.setSelectionCells([edge]);
                }
                else if (chg.constructor.name == MxGraphInnerClass_1.MxGraphInnerClass.mxGeometryChange) {
                    var change = chg;
                    var cell = change.cell;
                    _this.graph.bpmnCellGeometryChanged(cell, sender, evt, change);
                    if (flag && cell != null) {
                        _this.fireEvent(new mxgraph_ts_1.mxEventObject(PmfEvents_1.PmfEvents.ShowProperty, "cells", [cell]));
                        flag = false;
                    }
                }
            });
        ...
        // 元素删除事件处理
        this.graph.addListener(mxgraph_ts_1.mxEvent.CELLS_REMOVED, function (sender, evt) {
            var cells = evt.getProperty("cells");
            _this.graph.bpmnCellsRemoved(cells, sender, evt);
        });
        // 修改选中元素事件处理
        this.graph.getSelectionModel().addListener(mxgraph_ts_1.mxEvent.CHANGE, function (sender, evt) {
            var cells = _this.graph.getSelectionCells();
            _this.graph.bpmnCellsSelectionChanged(cells, sender, evt);
        });
        // 添加连线事件
        this.graph.connectionHandler.addListener(mxgraph_ts_1.mxEvent.CONNECT, function (sender, evt) {
            var edge = evt.getProperty("cell");
            _this.graph.bpmnCellsConnected(edge, sender, evt);
        });
        // 活动名称修改事件
        this.addListener(PmfEvents_1.PmfEvents.NameChanged, function (sender, evt) {
    ...
    EditorUi.prototype.destroy = function () {
        ...
        if (this.gestureHandler != null) {
            mxgraph_ts_1.mxEvent.removeGestureListeners(document, this.gestureHandler);
            this.gestureHandler = null;
        }
        if (this.resizeHandler != null) {
            mxgraph_ts_1.mxEvent.removeListener(window, 'resize', this.resizeHandler);
            this.resizeHandler = null;
        }
        if (this.deleteKeyHandler != null) {
            mxgraph_ts_1.mxEvent.removeListener(document, 'keydown', this.deleteKeyHandler);
