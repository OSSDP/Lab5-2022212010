...
var Sidebar = /** @class */ (function () {
    function Sidebar(editorUi, container) {
         ...
         * 提示信息图片
         */
        this.tooltipImageSrc = Constant_1.Constant.IMAGE_PATH + '/tooltip.png';
        /**
         * 提示信息显示延迟
         */
         ...
         * 分组折叠图片
         */
        this.collapsedImage = Constant_1.Constant.IMAGE_PATH + '/collapsed.gif';
        /**
         * 分组展开图片
         */
        this.expandedImage = Constant_1.Constant.IMAGE_PATH + '/expanded.gif';
        /**
         * 拖拽至画布后元素高度
         */
        ...
        if (!mxgraph_ts_1.mxClient.IS_IE && !mxgraph_ts_1.mxClient.IS_IE11) {
    ...
    Sidebar.prototype.createTemporaryGraph = function (stylesheet) {
        var graph = new mxgraph_ts_1.mxGraph(document.createElement('div'), null, null, stylesheet);
    ...
    Sidebar.prototype.reset = function (toolbox) {
        ...
        toolbox && toolbox.groups && toolbox.groups.forEach(function (g) {
            ...
            g.items && g.items.forEach(function (it) {
                var cell = BpmnCellHelper_1.BpmnCellHelper.generate(it.clrTypeId, undefined, new mxgraph_ts_1.mxGeometry(0, 0, _this.defaultImageWidth, _this.defaultImageHeight));
                if (cell.edge) {
                    cell.geometry.setTerminalPoint(new mxgraph_ts_1.mxPoint(0, _this.defaultImageHeight), true);
                    cell.geometry.setTerminalPoint(new mxgraph_ts_1.mxPoint(_this.defaultImageWidth, 0), false);
    ...
    Sidebar.prototype.createPaletteTitle = function (label) {
        ...
        elt.setAttribute('title', mxgraph_ts_1.mxResources.get('sidebarTooltip'));
        elt.className = 'geTitle';
        mxgraph_ts_1.mxUtils.write(elt, label);
        return elt;
    };
    /**
     * 添加工具箱显示项分组标题栏折叠处理器
    ...
    Sidebar.prototype.addFoldingHandler = function (title, content, onInit) {
        ...
        if (!mxgraph_ts_1.mxClient.IS_IE || document.documentMode >= 8) {
            title.style.backgroundImage = (content.style.display == 'none') ?
                'url(\'' + this.collapsedImage + '\')' : 'url(\'' + this.expandedImage + '\')';
        }
        title.style.backgroundRepeat = 'no-repeat';
        title.style.backgroundPosition = '0% 50%';
        mxgraph_ts_1.mxEvent.addListener(title, 'click', function (evt) {
            if (content.style.display == 'none') {
                if (!initialized) {
                    ...
                    if (onInit != null) {
                        ...
                        title.innerHTML = mxgraph_ts_1.mxResources.get('loading') + '...';
                        window.setTimeout(function () {
                            var fo = mxgraph_ts_1.mxClient.NO_FO;
                            mxgraph_ts_1.mxClient.NO_FO = Constant_1.Constant.ORIGINAL_NO_FO;
                            onInit(content);
                            mxgraph_ts_1.mxClient.NO_FO = fo;
    ...
    Sidebar.prototype.createItem = function (cells, title, showLabel, showTitle, width, height, allowCellsInserted) {
        ...
        var border = (mxgraph_ts_1.mxClient.IS_QUIRKS) ? 8 + 2 * this.thumbPadding : 2 * this.thumbBorder;
        elt.style.width = 70 + 'px';
        elt.style.height = 70 + 'px';
        // elt.style.padding = this.thumbPadding + 'px';
        if (mxgraph_ts_1.mxClient.IS_IE6) {
            elt.style.border = 'none';
        }
        // Blocks default click action
        mxgraph_ts_1.mxEvent.addListener(elt, 'click', function (evt) {
            mxgraph_ts_1.mxEvent.consume(evt);
        });
        this.createThumb(cells, this.thumbWidth, this.thumbHeight, elt, title, showLabel, showTitle, width, height);
        var bounds = new mxgraph_ts_1.mxRectangle(0, 0, width, height);
        if (cells.length > 0) {
        ...
        if (!mxgraph_ts_1.mxClient.IS_IOS) {
            mxgraph_ts_1.mxEvent.addGestureListeners(elt, null, function (evt) {
                if (mxgraph_ts_1.mxEvent.isMouseEvent(evt)) {
                    _this.showTooltip(elt, cells, bounds.width, bounds.height, title, showLabel);
                }
            });
            mxgraph_ts_1.mxEvent.addListener(elt, (mxgraph_ts_1.mxClient.IS_POINTER) ? 'pointerdown' : 'mousedown', function () {
                _this.hideTooltip();
                _this.showTooltips = false;
            });
            mxgraph_ts_1.mxEvent.addListener(elt, (mxgraph_ts_1.mxClient.IS_POINTER) ? 'pointerup' : 'mouseup', function () {
                _this.showTooltips = true;
            });
            mxgraph_ts_1.mxEvent.addListener(elt, (mxgraph_ts_1.mxClient.IS_POINTER) ? 'pointerleave' : 'mouseleave', function (evt) {
                _this.hideTooltip();
                _this.showTooltips = true;
            });
        }
    ...
    Sidebar.prototype.createThumb = function (cells, width, height, parent, title, showLabel, showTitle, realWidth, realHeight) {
        ...
        if (this.graph.dialect == mxgraph_ts_1.mxConstants.DIALECT_SVG && !mxgraph_ts_1.mxClient.NO_FO) {
        ...
        if (mxgraph_ts_1.mxClient.IS_IE6) {
            parent.style.backgroundImage = 'url(' + Constant_1.Constant.TRANSPARENT_IMAGE + ')';
        }
        ...
        if (this.sidebarTitles && title != null && showTitle != false) {
            var border = (mxgraph_ts_1.mxClient.IS_QUIRKS) ? 2 * this.thumbPadding + 2 : 0;
            // parent.style.height = (this.thumbHeight + border + this.sidebarTitleSize + 20) + 'px';
            // parent.style.width = (this.thumbWidth + border + 8) + 'px';
            var div = document.createElement('div');
            div.style.fontSize = mxgraph_ts_1.mxClient.IS_FF ? (this.sidebarTitleSize + 2) + 'px' : this.sidebarTitleSize + 'px';
            div.style.color = '#303030';
            div.style.textAlign = 'center';
            div.style.whiteSpace = 'nowrap';
            if (mxgraph_ts_1.mxClient.IS_IE) {
                div.style.height = (this.sidebarTitleSize + 12) + 'px';
            }
            div.style.paddingTop = '5px';
            mxgraph_ts_1.mxUtils.write(div, title);
            ...
            if (this.currentElt != elt) {
                ...
                var show = function () {
                    ...
                    if (_this.tooltip == null) {
                        ...
                        _this.tooltip.style.zIndex = (mxgraph_ts_1.mxPopupMenu.prototype.zIndex - 1).toString();
                        document.body.appendChild(_this.tooltip);
                        _this.graph2 = new mxgraph_ts_1.mxGraph(_this.tooltip, null, null, _this.editorUi.graph.getStylesheet());
                        ...
                        _this.tooltipImage = mxgraph_ts_1.mxUtils.createImage(_this.tooltipImageSrc);
                        _this.tooltipImage.className = 'geSidebarTooltipImage';
                        _this.tooltipImage.style.zIndex = (mxgraph_ts_1.mxPopupMenu.prototype.zIndex - 1).toString();
                    ...
                    if (mxgraph_ts_1.mxClient.IS_QUIRKS) {
                        height += 4;
                    ...
                    if (_this.tooltipTitles && title != null && title.length > 0) {
                        ...
                        mxgraph_ts_1.mxUtils.write(_this.tooltipTitle, title);
                    ...
                    var off = new mxgraph_ts_1.mxPoint(0, 0);
    ...
    Sidebar.prototype.createDragSource = function (elt, dropHandler, preview, cells, bounds) {
        var dragSource = mxgraph_ts_1.mxUtils.makeDraggable(elt, this.editorUi.graph, function (graph, evt, target, x, y) {
    ...
    Sidebar.prototype.createDropHandler = function (cells, allowSplit, allowCellsInserted, bounds) {
        ...
        var handler = function (graph, evt, target, x, y, force) {
            var elt = (force) ? null : ((mxgraph_ts_1.mxEvent.isTouchEvent(evt) || mxgraph_ts_1.mxEvent.isPenEvent(evt)) ?
                document.elementFromPoint(mxgraph_ts_1.mxEvent.getClientX(evt), mxgraph_ts_1.mxEvent.getClientY(evt)) :
                mxgraph_ts_1.mxEvent.getSource(evt));
            ...
            if (elt == null && graph.isEnabled()) {
                ...
                if (cells.length > 0) {
                    ...
                    var validDropTarget = (target != null && !mxgraph_ts_1.mxEvent.isAltDown(evt)) ?
                    ...
                    if (!graph.isCellLocked(target || graph.getDefaultParent())) {
                        ...
                        try {
                            ...
                            if (allowCellsInserted && (evt == null || !mxgraph_ts_1.mxEvent.isShiftDown(evt))) {
                                graph.fireEvent(new mxgraph_ts_1.mxEventObject(PmfEvents_1.PmfEvents.CellsInserted, 'cells', select));
                            }
                        }
                        finally {
